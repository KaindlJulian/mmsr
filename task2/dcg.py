...
def calculate_idcg(sample_query: pd.Series, k: int, genres: pd.DataFrame) -> float:
    ...
    """
    # Vectorized calculation of relevance scores
    relevance_scores = genres['genre'].apply(lambda x: sorensen_dice_genres(sample_query["genre"], x))
    # sort and take top k
...
def calculate_idcg_2(sample_query: str, k: int, genres: dict) -> float:
    relevance_scores = [sorensen_dice_genres(genres[sample_query], genres[x]) for x in genres.keys() if x != sample_query]
...
def calculate_dcg_2(sample_query: str, retrieved_k_tracks: pd.DataFrame, k: int, genres: dict) -> float:
    relevance_scores = [sorensen_dice_genres(genres[sample_query], genres[x]) for x in retrieved_k_tracks["id"] if x != sample_query]
